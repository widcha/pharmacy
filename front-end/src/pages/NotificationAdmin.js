...
const AllNotifAdmin = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchNotifAdmin(`${window.location.search}&select=true`));
  }, [dispatch]);
  const {notif, lengths} = useSelector((state) => state.admin);

  const [pageCount, setPageCount] = useState(lengths.notifunread / perPage);
  useEffect(() => {
    setPageCount(lengths.notifunread / perPage);
  }, [perPage]);

  const handlePageClick = (e) => {
    const selectedPage = e.selected;
    history.push(`/notifications?page=${selectedPage + 1}&limit=10`);
    dispatch(fetchNotifAdmin(`?page=${selectedPage + 1}&limit=10&select=true`));
  };
          ...
          <div class="w-full">
            <div class="bg-white shadow-md rounded">
              <table class="min-w-max w-full table-auto">
                <thead>
                  <tr class="bg-blue-200 text-gray-800 uppercase text-sm leading-normal">
      ...
      <div>
        ...
        <div>
          <div className="flex-col flex justify-center fixed right-10">
            ...
            <div className="pl-7">
              <Link to="/all-notifications?page=1&limit=10&all=true">
