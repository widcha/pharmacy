...
const CardProduct = ({ idProd, name, price, stock, vol, stock_total, desc, cat, image }) => {
    ...
    const saveChange = (id) => {
        if(newName && newPrice && newVol && newDesc && selectedCategory && newStock){
            dispatch(editProductAction({id, newName, newPrice, newVol, newStock, newDesc, selectedCategory, pict}));
        }
        else{
            Swal.fire({
                icon: 'warning',
                    ...
                    <form onSubmit={(e) => console.log(e)}>
                      ...
                      <div className="relative p-6 flex-auto" style={{ display: 'flex', flexDirection: 'column', width: '500px' }}>
                        ...
                        <div>Stock (per bottle):
                            <Button disabled={newStock === 1} onClick={() => setStock(newStock-1)}>-</Button>
                            {newStock}
                            <Button onClick={() => setStock(newStock+1)}>+</Button>
                        </div>
